      ...
      <AnimatePresence>
        {isCartOpen && (
          <motion.div
            ...
            className="fixed top-0 right-0 h-screen w-[320px] lg:w-[400px] bg-white z-[999] overflow-y-auto"
          >
            <button
            ...
            <div className="flex flex-col items-start gap-4 p-4 pt-16">
              {cartItems.length === 0 ? (
                <>
                  ...
                  <div className="flex flex-col items-start gap-4 mt-14 font-serif">
                    ...
                    <NavLink
                      className="text-[16px] leading-[26px] font-serif text-[#030303] border-b border-[#030303] hover:text-[#419338] font-roboto-serif hover:border-[#419338] py-1 transition-all duration-150"
                      to="/"
                      onClick={() => {
                        navigate("/");
                        closeCart();
                ...
                <>
                  ...
                  <div className="flex flex-col items-start gap-4 mt-4 w-full">
                    ...
                    {cartItems.map((item) => (
                      <div
                        key={item.id}
                        className="w-full flex items-start gap-2 border-b pb-2"
                      >
                        <img
                          src={item.image}
                          alt={item.name}
                        ...
                        <div className="flex flex-col gap-1 flex-1">
                          ...
                          <button
                            className="outline-none bg-transparent cursor-pointer text-red-500 text-xs"
                            onClick={() => removeFromCart(item.id)}
                          >
                            Remove
                          </button>
                        </div>
                        {/* FIX: per-item total, not global total */}
                  ...
                  <div className="w-full flex flex-col items-center justify-end gap-3 mt-4">
                    <div className="flex justify-between w-full px-4">
                      <span className="text-[16px] leading-[26px] text-[#030303] font-normal">
                        Subtotal
                      </span>
                      <span className="text-[16px] leading-[26px] text-[#030303] font-normal">
                    ...
                    <p className="text-[16px] leading-[21px] text-[#030303] hover:text-[#419338] transition-all duration-100 font-normal">
                      Taxes and{" "}
                      <span className="border-b border-[#030303]">
                        shipping
                      </span>{" "}
                      calculated at checkout
                    </p>
                    <button
                      className="bg-[#419338] cursor-pointer border border-[#030303] bg-[#030303] text-white w-full py-2 hover:text-black transition-all duration-150"
                      onClick={() => {
                        // FIX: use correct checkout route (match your router)
                        navigate("/payment");
                        closeCart();
                      }}
                    >
